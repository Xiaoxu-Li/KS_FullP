% Perturbation-based method to evaluate error indicator
function [err_post, err_post2, err_post12, err_post22] = EcutestiGP(L, Ec, Neig, Eg, phi, beta, rho)
err_post=zeros(Neig,1);
err_post2 = zeros(Neig,1);
err_post12 = zeros(Neig,1);
err_post22 = zeros(Neig,1);
...
for R = 0:Ns
 for ii = -Ns:Ns
    for j = -Ns:Ns
         ...
         invlapres_fft12 = real(res(:,:,ll)) .* weight2;
         res_weight = reshape(real(invlapres_fft.*res(:,:,ll)), s(1)^2, 1);
         res_weight12 = reshape(real(invlapres_fft12.*res(:,:,ll)), s(1)^2, 1);
         invlapres_fftk = zeros(dofg,1);
         invlapres_fftk12 = zeros(dofg,1);
      m=0;
      for R = 0:Ng
        for ii = -Ng:Ng
            for j = -Ng:Ng
                ...
                if R-1 < r2 && r2 <= R
                        ...
                        invlapres_fftk12(m) = invlapres_fft12(iik, jk);
                end
            end
        end
      end
...
%         Vinvlapres22 = (Hv - Hpiv) * invlapres_fftk12;
%         res_weight2 = invlapres_fftk'*Vinvlapres;
%         res_weight22 = invlapres_fftk12'*Vinvlapres22;
...
%         err_post22(ll) = err_post12(ll) - real(res_weight22); % old 2nd
         err_post2(ll) = 0;
         err_post22(ll) = 0;
end
return