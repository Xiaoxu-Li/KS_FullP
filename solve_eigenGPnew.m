% solve the linearizational GP eigenvalue problem :
...
% lambda: eigenvalues
% phi: eigenfunctions
function [lambda,phi] = solve_eigenGPnew(L, Ec, Neig, Eg, beta, rho)
% initiate the degrees of freedom
Ng = floor((L/pi)*sqrt(2*Eg)); % radius (to calculate the potential element)
Nc = floor((L/pi)*sqrt(2*Ec)); % radius
dof = 0;
dofg = 0;
for ii = -Nc:Nc
    for j = -Nc:Nc
