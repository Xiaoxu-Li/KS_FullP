function [lambda,phi]=eigen_dg_per_slater(L, R, N_p, n_r, Lm, sigma)
% eigen_dg_per_slater solve the eigenvalue problem with  periodic potential 
...
%
% using slater orbitals as radial basis functions
% Aug 20th, 2018
...
% evaluate discrete periodic potential 
mm=4*N+1;
vext = zeros(mm,mm,mm);
for ii = 1:mm
    for j = 1:mm
    ...
    for q=1:n0
        ...
        if p==q   
            ...
            for j=1:N0
                ...
                v_per=-4*pi/(K^2*volum_omega)+sum_per*(4*pi)^2/(volum_omega)^2 ...
                    +4*pi*volum_in/(K^2*volum_omega^2);    
        end
        H(p,q) = H(p,q) + v_per;
