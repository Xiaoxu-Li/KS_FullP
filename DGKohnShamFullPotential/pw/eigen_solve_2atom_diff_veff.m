function [lambda,phi]=eigen_solve_2atom_diff_veff(L, Nc)
% eigen_solve_2atom solve the eigenvalue problem with periodic potential such
% as LiH
...
for ii=1:n
    for j=1:n
        ...
        if p(ii,:) == p(j,:)
            ...
            H(ii,j)=-(2*shift1+shift2)*4*pi/(L^3*norm(dk,2)^2*(2*pi/L)^2);   % LiH 
        end         
    end
end
