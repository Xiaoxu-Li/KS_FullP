 function [lambda_new,phi]=eigen_dg_nonlinear1(L, R, N_p, n_r, Lm, sigma)
% eigen_solve_nonlinear solve the nonlinear eigenvalue problem   
% (-1/2\Delta+V_{ext}+V_H{\rho})u=\lambda u [\lambda,\phi] returns the eigenpair
...
% SCF iteration through simple iteration   \rho_in=\rho_out
% load jump data 
% Aug 20th, 2018
    ...
    for q=1:n0
        ...
        H(p,q)=H(p,q)+2*vext(index(1),index(2),index(3));     % external potential of He
         ...
         if kp == 0 && kq == 0
             ...
             A(p,q) = A(p,q) + 4*pi*R^2 * sigma / volum_omega;
         ...
         elseif kp > 0 && kq == 0
             ...
             A(p,q) = A(p,q) + 4*pi * 0.25*R^2/volum_omega * (2*djp+ 4*sigma * jp);
         elseif kp == 0 && kq > 0
             ...
             A(p,q) = A(p,q) + 4*pi * 0.25*R^2/volum_omega * (2*djq + 4*sigma * jq);
         % case kp>0 and kq>0    
         elseif kp > 0 && kq > 0
%              sum = 0.0;
%              for l = 0:lmax
                    ...
                    %-----------------------------------------------------------------------------------------------------------------%    
                        ...
                        if l1==l2 && m1==m2
                            ...
                            H(p,q) = H(p,q) + 0.25*(rp*drq +drp*rq + 4*sigma*rp*rq) * R^2; 
                             A(p,q) = A(p,q) + 0.25*(rp*drq +drp*rq + 4*sigma*rp*rq) * R^2;
                        end
                    end
                end
            end
...
coef_original=zeros(2*N+1,2*N+1,2*N+1);
 for p=1:n0
     ...
     for j1=1:2*N+1
         for k1=1:2*N+1
            for i2=1:2*N+1
                for j2=1:2*N+1
                    for k2=1:2*N+1
                        coef_new(i1-i2+2*N+1, j1-j2+2*N+1, k1-k2+2*N+1)=coef_new(i1-i2+2*N+1, j1-j2+2*N+1, k1-k2+2*N+1)+...
...
% [1] planewaves part
 ...
 for ii=1:n0
     ...
     k_index=kk(ii,:)+2*N+1;
     f(ii)=coef_new(k_index(1),k_index(2),k_index(3))*4*pi*volum_out/volum_omega...
            - sum_per*(4*pi)^2/volum_omega;             
 end
                         ...
                         for k2 = 1:n_r
                            for l2 = 0:Lm
                                for m2 = -l2:l2
                                    p = n0 + (k1-1)*(Lm+1)^2 + l1^2 + m1+l1+1;
                                    q = n0 + (k2-1)*(Lm+1)^2 + l2^2 + m2+l2+1;
                                    f(ii)=f(ii)+c(p)*c(q)'/nrm^2*int_3(k1,k2,k3)*(-1)^(m2)*sqrt((2*l1+1)*(2*l2+1)*(2*l3+1))...
...
f=2*f;   % \rho=2*|u|^2  including spin
...
% save dg_nonlinear L R n0 N_p n_r Lm c nrm kk volum_omega delta
