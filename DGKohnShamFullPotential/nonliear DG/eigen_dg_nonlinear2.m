function [lambda_new,phi]=eigen_dg_nonlinear2(L, R, N_p, n_r, Lm, sigma)
...
% SCF iteration through simple mixing for \rho_in=1/10*(9*\rho_in+\rho_out)
% Aug 20th, 2018

% deal with the penalization exactly without using jump data, but too much
% cost
...
N1=floor(N_cut);
% N_cut=2*N;    %  if N_p is half integer, N1=2*N+1 maybe beyond the index in SCF iteration
% N1=floor(N_cut);
a=zeros(8*N1^3,3);
for ii=-N1:N1
      m=sqrt(N_cut^2-ii^2);
...
for p=1:n0
    for q=1:n0
        ...
        % case kp>0 and kq>0            
        % without using jump data in order to deal with the penalization
        % term exactly
        elseif kp > 0 && kq > 0
             sum = 0.0;
             sum1=0.0;
             for l = 0:lmax
                 ...
                 sum1 = sum1 + (-1)^l*s_m*(2*jp*djq +2*djp*jq + 4*sigma*jp*jq);     % not exactly twice of sum, but we can ignore this little error and use jump data
             end
             H(p,q) = H(p,q) + 0.25*(4*pi)^2*sum * R^2/volum_omega;
             A(p,q)=A(p,q)+0.25*(4*pi)^2*sum1 * R^2/volum_omega;
         end
    end
end
