function [lambda,phi]=eigen_dg_2atom_multi(L, R, N_p, n_r1,n_r2, Lm1,Lm2, sigma)
% solve the eigenvalue problem of two-atom system with the periodic potential
...
for p=1:n0
    for q=1:n0
        ...
        H(p,q)=H(p,q)+vext(index(1),index(2),index(3));       
         ...
         % case kp>0 and kq>0    
         elseif kp > 0 && kq > 0
              j_index=[kk(p,1),kk(p,2),kk(p,3),kk(q,1),kk(q,2),kk(q,3)]+5+1;    % +5 here due to the size of jump_data
