function [lambda,phi]=eigen_dg_2atom_diff_v(L, R, N_p, n_r1,n_r2, Lm1,Lm2, sigma)
% solve the eigenvalue problem for two-atom system with periodic potential of different constant  2/r+1/r'
% such as LiH
            ...
            shift2=exp(1i*k_index*R2'*2*pi/L);
                if k_norm==0   
                    for s=1:N0
                        ...
                        sum_per=sum_per+(2+exp(1i*kkk(s,:)*(R1-R2)'*2*pi/L)+2*exp(1i*kkk(s,:)*(R2-R1)'*2*pi/L)) ...
                            *(sin(k_per*R)/k_per^5-cos(k_per*R)*R/k_per^4) ;                           
                    end
                    ...
                    for s=1:N0
                        ...
                        (2*shift1+shift2+2*shift2*exp(1i*kkk(s,:)*(R2-R1)'*2*pi/L)+shift1*exp(1i*kkk(s,:)*(R1-R2)'*2*pi/L));
                    end
                    v_per=4*pi/(volum_omega)^2 * ( 4*pi* sum_per+(4*shift1+2*shift2)*volum_in/k_norm^2);                            
                    v_per=-4*pi/(k_norm^2*volum_omega) *(2*shift1+shift2) +v_per;
                end                    
    ...
    for k2 = 1:n_r1
       ...
       % periodic potential term
        ...
        for j=1:N0
            ...
            sum_per_in=sum_per_in+(2+exp(1i*kkk(j,:)*(R1-R2)'*2*pi/L))*g(k1,k2,j);               
        end            
    ...
    for k2 = 1:n_r2
       ...
       % periodic potential term 
        ...
        for j=1:N0
            ...
            sum_per_in=sum_per_in+(1+2*exp(1i*kkk(j,:)*(R2-R1)'*2*pi/L))*g(k1,k2,j);   
        end            
